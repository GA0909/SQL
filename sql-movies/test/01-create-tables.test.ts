...
CREATE INDEX IF NOT EXISTS movie_ratings_time_created_idx
ON ${MOVIE_RATINGS} (time_created);
`;

const CREATE_UNIQUE_INDEX_MOVIES_IMDB_ID = `
CREATE UNIQUE INDEX IF NOT EXISTS movies_imdb_id_unq_idx
ON ${MOVIES} (imdb_id);
`;

const CREATE_UNIQUE_INDEX_KEYWORDS_KEYWORD = `
CREATE UNIQUE INDEX IF NOT EXISTS keywords_keyword_unq_idx
ON ${KEYWORDS} (keyword);
`;

const CREATE_UNIQUE_INDEX_GENRES_GENRE = `
CREATE UNIQUE INDEX IF NOT EXISTS genres_genre_unq_idx
ON ${GENRES} (genre);
`;

const CREATE_UNIQUE_INDEX_PRODUCTION_COMPANIES_COMPANY_NAME = `
CREATE UNIQUE INDEX IF NOT EXISTS production_companies_company_name_unq_idx
